...
const qrCodeViewer: React.FC<QrCodeViewerProps> = ({ imagePath, closeQr, isVisible }) => {
  const handleClose = () => {
    closeQr();
  };
    ...
    <>
      <dialog className={'qrcode-dialog'} id="dialog" open={isVisible}>
        <div className="dialog-buttons">
          <button id="download" data-tooltip-id="downlaod-tooltip" data-tooltip-content="Download" onClick={downloadQrCode}>
            {' '}
            <i className="pi pi-file-o"></i>
          </button>
          <button id="close" data-tooltip-id="close-tooltip" data-tooltip-content="Close" onClick={handleClose}>
        ...
        <QRCodeCanvas
          ...
          size={200}
          level={'H'}
          imageSettings={isNhnUrl ? { src: './public/nhnlogo.png', height: 40, width: 40, excavate: true } : undefined}
        />
      </dialog>
