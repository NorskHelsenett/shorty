...
const EditableRow: React.FC<EditableRowProps> = ({ data, onCancel, onUpdate, message }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="list-item">
        ...
        <div className="list-item__actions">
          <button type="submit" data-tooltip-id="save-tooltip" data-tooltip-content={'Save changes'}>
            Save
          </button>
          <button onClick={onCancel} data-tooltip-id="cancel-tooltip" data-tooltip-content={'Cancel changes'}>
            Cancel
          </button>
        </div>
      </div>
      {localError && <p className="warning">{localError}</p>}
      {message && <p className={message.type === 'success' ? 'success ' : 'warning'}>{message.message}</p>}
    </form>
  );
};
